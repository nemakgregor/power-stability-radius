...
def compute_ac_l2_radius(
    ...
    *,
    ...
    # slack like 0 with an ext_grid elsewhere shifts every expanded h_P entry
    ...
    for zero_start in range(0, int(zero_indices.size), int(chunk_size)):
        ...
        for local_pos, con_idx_raw in enumerate(zero_chunk):
            ...
            gram = np.asarray(
                [
                    [
                        np.dot(projected_p, projected_p),
                        np.dot(projected_p, projected_q),
                    ],
                    [
                        np.dot(projected_q, projected_p),
                        np.dot(projected_q, projected_q),
                    ],
                ],
                dtype=float,
            )
    ...
    for pos, lid in enumerate(line_ids):
        ...
        results[k] = {
            ...
            "zero_flow_threshold_mva": float(
                zero_flow_threshold_mva[
                    2 * pos if binding_end == "from" else 2 * pos + 1
                ]
            ),
    ...
    if return_h_vectors:
        results["_h_vectors"] = {
            ...
            "adjoint_residual_max": float(adjoint_max_relative_residual),
        }
