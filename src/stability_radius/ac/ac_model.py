...
def build_ac_operator(
    *,
    ...
    try:
        ...
        raise ValueError("Could not map pandapower buses into the PYPOWER case.") from exc
    ...
    dSf_dVa, dSf_dVm, dSt_dVa, dSt_dVm, _Sf, _St = dSbr_dV(
        ppc["branch"], Yf, Yt, V_ppc
    )

    mask_non_slack = np.ones(n_bus, dtype=bool)
    mask_non_slack[int(slack_pos)] = False
    pv_mask = np.asarray(
        [int(ppc["bus"][row, BUS_TYPE]) not in {int(PQ), int(REF)} for row in ppc_bus_pos],
        dtype=bool,
    )
