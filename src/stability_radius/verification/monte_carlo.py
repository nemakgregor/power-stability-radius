...
- "Coverage" terminology is avoided: we report
    * P_safe under a chosen distribution (Gaussian in reduced coords)
    * certified ball mass (analytic + MC)
    * soundness check inside the certified ball (hard check)

Distribution used
-----------------
We work in reduced balanced coordinates of dimension d = n_bus - 1
(i.e. injections on non-slack buses; slack balances implicitly in the DC operator).
...
This matches the certificate computation in this project where H is defined in the
same reduced coordinates (slack eliminated).
...
def _sample_gaussian(
    *, rng: np.random.Generator, n: int, d: int, sigma_mw: float
) -> np.ndarray:
    """Sample Δp_red ~ N(0, σ² I_d), shape (n, d)."""
    if n <= 0:
    ...
    return (s * rng.standard_normal(size=(int(n), int(d)))).astype(float, copy=False)


def _sample_uniform_l2_ball(
    *, rng: np.random.Generator, n: int, d: int, radius: float
) -> np.ndarray:
    """
    Sample uniformly in the L2 ball {x in R^d: ||x||_2 <= radius}.
    ...
    if r == 0.0:
        return np.zeros((int(n), int(d)), dtype=float)

    z = rng.standard_normal(size=(int(n), int(d))).astype(float, copy=False)
    ...
    while bool(np.any(bad)):
        ...
        z_bad = rng.standard_normal(size=(k_bad, int(d))).astype(float, copy=False)
        z[bad, :] = z_bad
        norms = np.linalg.norm(z, ord=2, axis=1)
        bad = norms <= 1e-12
...
def _run_gaussian_stage(
    *,
    ...
    while remaining > 0:
        ...
        delta_red = _sample_gaussian(rng=rng, n=k, d=d, sigma_mw=float(sigma_mw))
        norms = np.linalg.norm(delta_red, ord=2, axis=1)
        in_ball_mask = norms <= float(r_star)
        in_ball += int(np.sum(in_ball_mask))

        df = _flows_from_delta_injections_reduced(dc_op=dc_op, delta_red=delta_red)
        viol = df + f0[None, :]
        ...
        if batch_worst > worst_max_violation:
            ...
            worst_line_pos = lp
            worst_line_idx = (
                int(sorted(dc_op.line_ids)[lp]) if 0 <= lp < len(dc_op.line_ids) else -1
            )
...
def _run_soundness_stage(
    *,
    ...
    while remaining > 0:
        ...
        delta_red = _sample_uniform_l2_ball(rng=rng, n=k, d=d, radius=float(r_star))
...
def run_monte_carlo_verification(
    *,
    ...
    # sigma is taken from results meta (must match how radii were computed)
    ...
    # Gaussian stage
    with log_stage(logger, "Monte Carlo: Gaussian safety (reduced balanced coords)"):
        (
            gauss_feasible,
            gauss_feas_ci,
            gauss_in_ball,
...
def estimate_coverage_percent(**kwargs: Any) -> dict[str, Any]:
    """
    ...
    -------
    dict
        A JSON-friendly dict of VerificationResult.
    """
    vr = run_monte_carlo_verification(**kwargs)
    return vr.to_dict()
