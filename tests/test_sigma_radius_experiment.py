...
def _make_res(
    ...
    from stability_radius.experiments.run_sigma_radius import _build_result_dict
...
class TestBuildTable2Rows:
    def test_returns_all_rows(self) -> None:
        from stability_radius.experiments.run_sigma_radius import _build_table2_rows
...
class TestWorstCaseVerificationSkipsInfeasible:
    def test_negative_r_sigma_lines_are_skipped(self) -> None:
        ...
        from stability_radius.experiments.run_sigma_radius import (
            _build_table2_rows,
        ...
        with (
            patch("stability_radius.experiments.run_sigma_radius.copy") as mock_copy,
            patch("stability_radius.experiments.run_sigma_radius.verify_worst_case") as mock_verify,
            patch.object(Path, "open", create=True),
            patch("stability_radius.experiments.run_sigma_radius.json"),
        ):
...
class TestSaveHvectorsNPZ:
    def test_hvectors_saved_and_loadable(self, tmp_path: Path) -> None:
        from stability_radius.experiments.run_sigma_radius import _save_hvectors_npz
