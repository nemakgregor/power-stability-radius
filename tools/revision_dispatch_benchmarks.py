...
def _generator_model(net: Any) -> tuple[list[tuple[str, int, int]], np.ndarray, np.ndarray]:
...
def _preventive_dc_scopf(
    *,
    ...
    limits = np.asarray(
        [
            0.99
            * float(estimate_line_limit_mva_with_flag(model, model.line.loc[line_id])[0])
    ...
    a_ub = sp.vstack([flow_sparse, sp.csr_matrix(np.vstack(tangent_rows))], format="csr")
    ...
    return solved, _cost_from_solved_network(solved), len(lodf_result.islanded_contingencies)
    ...
    for case_name in args.cases:
        ...
        for load_scale in args.load_scales:
            ...
            try:
                ...
                for method in ("radius", "loading", "headroom", "sensitivity", "ptdf", "chance"):
                        ...
                        rows.append(
                            {
                                ...
                                "status": "ok",
                                "cost": float(cost),
                                "cost_increase_percent": 100.0 * (float(cost) / base_cost - 1.0),
                    ...
                    rows.append(
                        {
                            ...
                            "cost_increase_percent": 100.0 * (float(cost_scopf) / base_cost - 1.0),
