        ...
        rows.append(
            {
                ...
                "n_bus": n_bus,
                "eps_mva": float(eps_mva),
                "radius_ac_l2": row.get(f"radius_ac_l2_{line_end}", float("nan")),
                "h_norm_reported": row.get(f"ac_norm_a_{line_end}", float("nan")),
...
def _fd_summary_rows(fd_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for case in cases:
        ...
        absolute = [
            ...
            and math.isfinite(
                float(row.get("absolute_error_mva_per_mva", float("nan")))
            )
        ]
        normalized = [
        ...
        out.append(
            {
                ...
                "random_ends": sum(
                    1
                    for row in rows
                    if row.get("status") == "ok" and row.get("sample_type") == "random"
                ),
                "all_ends": sum(
                    1
                    for row in rows
