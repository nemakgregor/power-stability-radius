...
def _flow_components(
    ...
    p = self_g * vm_i**2 + vm_i * vm_k * (
        cross_g * anp.cos(delta) + cross_b * anp.sin(delta)
    )
    q = -self_b * vm_i**2 + vm_i * vm_k * (
        cross_g * anp.sin(delta) - cross_b * anp.cos(delta)
    )
...
def _case_rows(case_path: Path) -> list[dict[str, Any]]:
    ...
    ppc["bus"][ppc_bus_pos, VA] = np.rad2deg(np.asarray(base_pf.v_ang_rad, dtype=float))
    voltage = ppc["bus"][:, VM] * np.exp(1j * np.deg2rad(ppc["bus"][:, VA]))
    _ybus, yf, yt = makeYbus(base_mva, ppc["bus"], ppc["branch"])
    dSf_dVa, dSf_dVm, dSt_dVa, dSt_dVm, sf, st = dSbr_dV(ppc["branch"], yf, yt, voltage)
    ...
    for local_pos, line_id in enumerate(net_line_order):
        ...
        for line_end in ("from", "to"):
            ...
            coefficients = {
                int(col): complex(value) for col, value in zip(yrow.indices, yrow.data)
            }
            ...
            rows.append(
                {
                    ...
                    "line_id": int(line_id),
                    "line_end": line_end,
                    "max_abs_error_p": float(
                        np.max(np.abs(ad_jacobian[0] - analytic_p))
                    ),
                    "max_abs_error_q": float(
                        np.max(np.abs(ad_jacobian[1] - analytic_q))
                    ),
...
def run(args: argparse.Namespace) -> None:
    ...
    finite_s = [
        row["max_abs_error_s"]
        for row in all_rows
        if math.isfinite(row["max_abs_error_s"])
    ]
    summary = {
