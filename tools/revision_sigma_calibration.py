...
def _response_model(
    *,
    ...
    if model == "conditioned_balanced":
        ...
        p_scale = np.asarray(
            [
                max(0.5, 0.015 * abs(float(net.res_bus.loc[bus_id, "p_mw"])))
        ...
        q_scale = np.asarray(
            [
                max(0.25, 0.015 * abs(float(net.res_bus.loc[bus_ids[pos], "q_mvar"])))
    ...
    for _, row in net.gen.iterrows():
        ...
        if bus_id not in bus_pos:
            continue
        headroom = max(
            float(row.get("max_p_mw", 0.0)) - float(row.get("p_mw", 0.0)), 0.0
        )
        if headroom > 0.0:
...
def _case_model_rows(
    *,
    ...
    binding_ends = [
        str(results[f"line_{line_id}"]["binding_end"]) for line_id in line_ids
    ]
    ...
    nonlinear_flows = (
        np.vstack(all_flows) if all_flows else np.empty((0, len(line_ids)))
    )
    ...
    for pos, line_id in enumerate(line_ids):
        predicted_prob = (
            1.0 - normal.cdf((limits[pos] - base_flows[pos]) / analytical_sd[pos])
    ...
    metadata.update(
        {
            ...
            "bonferroni_sum_of_marginals": float(
                min(
                    1.0,
                    sum(
                        float(row["predicted_rating_exceedance_probability"])
                        for row in rows
                    ),
                )
            ),
...
def _summary(
    ...
    two_sigma = np.asarray(
        [
            float(row["empirical_exceedance_at_affine_two_sigma_threshold"])
            for row in rows
        ],
        dtype=float,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir", default="run_artifacts/revision1_sigma_calibration"
    )
    parser.add_argument(
        "--cases",
        nargs="+",
