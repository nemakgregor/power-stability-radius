...
def _add_near_zero_flow_spur(net) -> int:
    ...
    spur_bus = int(pp.create_bus(net, vn_kv=vn_kv, name="revision_near_zero_flow_bus"))
    line_id = int(
...
def run(args: argparse.Namespace) -> None:
    ...
    if bool(args.feasible_restoration):
        ...
        for pf_init in ("dc", "flat"):
            ...
            raise RuntimeError(
                "raw base power flow failed for dc and flat starts"
            ) from last_error
    rows = []
    for tolerance in args.relative_tolerances:
        ...
        line_rows = [row for key, row in results.items() if key.startswith("line_")]
        valid_radii = [
        ...
        rows.append(
            {
                ...
                "unresolved_nondifferentiable_binding_lines": int(
                    sum(
                        ...
                        and not bool(
                            row.get("zero_flow_operator_norm_certified", False)
                        )
                    )
                ),
