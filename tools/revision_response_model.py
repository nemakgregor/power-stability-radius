...
def run(args: argparse.Namespace) -> None:
    ...
    sigma_flow_sq = float(scalar_response @ primitive_covariance @ scalar_response.T)
    ...
    for index, bound in enumerate(primitive_bounds):
        coordinate_sigma = math.sqrt(
            max(float(primitive_covariance[index, index]), 0.0)
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir", default="run_artifacts/revision1_response_model"
    )
